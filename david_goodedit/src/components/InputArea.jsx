...
class InputArea extends React.Component {
  ...
  downloadFile() {
    if (this.state.transformedText === '') {
      return alert(
        "Télécharger un fichier vide ? Sérieux ?! David Goodenough a travaillé sur un truc qui s'appelle MARKDOWN, à gauche là. Servez vous en !"
      )
    } else {
      downloadjs(
    ...
    return (
      <div id='converterMk' className='InputContainer'>
        <section className='SecondInputContainer inset flex-column'>
          <header>
            <h2 style={{ display: 'inline-block' }} className='stack inline'>
              MARKDOWN
            </h2>
            (
            <Link
